...
class Branch:
    ...
    def version(self):
        return "{}.{}{}".format(
            self.major, self.minor, "." + str(self.micro) if self.micro else ""
        )
...
def rpm_is_kmod(filename):
    return (
        filename.startswith(KMOD_PKG_PREFIX) and not "dkms" and not "open" in filename
    )
...
def rpm_is_pkgname(rpm, pkgname, pkgversion=""):
    ...
    if pkgversion == "":
        ...
        return (
            rpm.startswith(pkgname) and pkgversion in rpm and rpm_stops == pkg_stops + 2
        )
...
if __name__ == "__main__":
    ...
    rpm_files = [
        f for f in repodir_contents if f.endswith(".rpm") and isfile(join(repodir, f))
    ]
    ...
    for pkg in driver_rpms:
        ...
        if n_branches == 0 or (
            n_branches > 0 and branches[n_branches - 1].major != major
        ):
    ...
    if len(branches) == 0:
        print(
            "Error: Could not determine branches from the given rpm files in " + repodir
        )
    ...
    latest_branch = Branch(
        "latest", latest.major, latest.minor, latest.micro, latest.arch
    )
    branches.insert(0, latest_branch)
    print("Latest Branch: " + latest_branch.version())
    latest_dkms_branch = Branch(
        "latest-dkms", latest.major, latest.minor, latest.micro, latest.arch
    )
    ...
    if int(latest.major) > 510:
        open_dkms_branch = Branch(
            "open-dkms", latest.major, latest.minor, latest.micro, latest.arch
        )
    ...
    for branch in branches:
        ...
        out.tab().line(
            "context: "
            + get_stream_hash("nvidia-driver", branch.name, time_stamp, distro)
        )
        ...
        for pkg in LATEST_PKGS:
            ...
            if latest_pkg:
                out.tab().tab().tab().line(
                    "- " + filename_to_nevra(latest_pkg, repodir)
                )
            else:
                print(
                    "WARNING: No package "
        ...
        if branch.is_open():
            open_pkg = latest_rpm_from_pkgname(
                rpm_files, "kmod-nvidia-open-dkms", branch.version()
            )
            if open_pkg:
                out.tab().tab().tab().line("- " + filename_to_nevra(open_pkg, repodir))
            else:
                print(
                    "WARNING: RPM kmod-nvidia-open-dkms in version "
                    + branch.version()
                    + " not found"
                )
        elif branch.is_dkms():
            dkms_pkg = latest_rpm_from_pkgname(
                rpm_files, "kmod-nvidia-latest-dkms", branch.version()
            )
            if dkms_pkg:
                out.tab().tab().tab().line("- " + filename_to_nevra(dkms_pkg, repodir))
            else:
                print(
                    "WARNING: RPM kmod-nvidia-latest-dkms in version "
                    + branch.version()
                    + " not found"
                )
        else:
            # All the kmod rpms which belong to this branch
            branch_kmod_rpms = list(
                filter(lambda r: kmod_belongs_to(r, branch), kmod_rpms)
            )
            if not branch_kmod_rpms:
                print(
                    "WARNING: Branch %s in version %s is not a DKMS branch, but no precompiled kmod packages can be found"
        ...
        if branch.is_open():
            ...
            out.tab().tab().tab().line(
                "description: Source files for compiling open kernel modules"
            )
            out.tab().tab().tab().line("rpms:")
        ...
        if branch.arch == "x86_64":
            ...
            if "latest" in branch.name:
                ...
                out.tab().tab().tab().tab().line(
                    "- " + "libnvidia-nscq-" + latest.major
                )
            elif int(branch.major) < 460:
                out.tab().tab().tab().tab().line(
                    "- " + "nvidia-fabricmanager-" + branch.major
                )
                out.tab().tab().tab().tab().line(
                    "- " + "libnvidia-nscq-" + branch.major
                )
            else:
                out.tab().tab().tab().tab().line("- " + "nvidia-fabric-manager")
                out.tab().tab().tab().tab().line(
    ...
    if len(outfile) > 0 and os.path.isfile("/usr/bin/modulemd-validator"):
        ...
        process = subprocess.Popen(
            ["/usr/bin/modulemd-validator", outfile],
